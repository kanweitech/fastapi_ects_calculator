...
with st.form("ects_form"):
    ...
    masters_program_academic_year = st.number_input("ECTS per Academic Year", min_value=1.0, step=1.0)
    number_of_years_of_bachelor_study = st.number_input("Years of Bachelor Study", min_value=1.0, step=1.0)
    total_number_of_units = st.number_input("Total Number of Units", min_value=1.0, step=1.0)
    name_of_course = st.text_input("Course Name")
    accumulated_units = st.number_input("Accumulated Units", min_value=0.0, step=1.0)
